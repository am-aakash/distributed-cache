...
public class Cache<KEY, VALUE> {
    ...
    private CompletionStage<VALUE> getFromCache(KEY key) {
        ...
        if (!cache.containsKey(key)) { // if not in local, go for db
            result = addToCache(key, loadFromDB(dataSource, key));
        } else { // if in db
            result = cache.get(key).thenCompose(record -> {
                if (hasExpired(record)) { // if expired
                    priorityQueue.get(record.getAccessDetails()).remove(key);
                    expiryQueue.get(record.getInsertionTime()).remove(key);
                    eventQueue.add(new Eviction<>(record, Eviction.Type.EXPIRY, timer.getCurrentTime())); // expire from local
                    return addToCache(key, loadFromDB(dataSource, key)); // go to db for the key
                } else { // if not expired return the record found
